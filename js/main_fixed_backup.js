...
// Function to restore logs with collapsing functionality
function restoreLogs() {
    logsContainer.innerHTML = '';
    const maxVisibleLogs = 10;
    ...
    function createToggleButton() {
        ...
        btn.className = 'mt-2 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition';
        btn.addEventListener('click', () => {
            if (showingAll) {
                renderLogs(userData.logs.slice(0, maxVisibleLogs));
                btn.textContent = 'Show More';
                showingAll = false;
            } else {
    ...
    if (totalLogs > maxVisibleLogs) {
        ...
        logsContainer.parentNode.insertBefore(toggleBtn, logsContainer.nextSibling);
    } else {
        renderLogs(userData.logs);
        // Remove toggle button if exists
        const existingBtn = logsContainer.parentNode.querySelector('button');
